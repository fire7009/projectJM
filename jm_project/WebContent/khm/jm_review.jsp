...
<body id="page-top">
 ...
 %>
								<%=boardReview.getFrstRgstDttm().substring(11, 19)%> <%=boardReview.getFrstRgstDttm().substring(0, 19)%>
							</td>
					...
					<div>
						...
						%>
						<a
							href="<%=request.getContextPath()%>/index.jsp?workgroup=khm&work=jm_review&pageNum=1&search=<%=search%>&keyword=<%=keyword%>">[◀◀]</a>
						<a
							href="<%=request.getContextPath()%>/index.jsp?workgroup=khm&work=jm_review&pageNum=<%=startPage - blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[◀]</a>
						<%
							} else {
						%>
						[◀◀][◀]
						...
						%>
						<a
							href="<%=request.getContextPath()%>/index.jsp?workgroup=khm&work=board_list&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>">[<%=i%>]
						</a>
						<%
							} else {
						%>
						...
						<a
							href="<%=request.getContextPath()%>/index.jsp?workgroup=khm&work=jm_review&pageNum=<%=startPage + blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[▶]</a>
						<a
							href="<%=request.getContextPath()%>/index.jsp?workgroup=khm&work=jm_review&pageNum=<%=totalReviewPage%>&search=<%=search%>&keyword=<%=keyword%>">[▶▶]</a>
						<%
							} else {
						%>
						[▶][▶▶]
				...
				<form action="<%=request.getContextPath()%>/index.jsp?workgroup=khm&work=jm_review" method="post">
					<select name="search">
...
$("#writeBtn").click(function() {
	location.href = "<%=request.getContextPath()%>/index.jsp?workgroup=khm&work=jm_review";
});
</script>
